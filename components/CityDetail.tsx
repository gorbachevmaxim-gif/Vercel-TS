            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x" style={{ borderColor: "transparent" }}>
                <div className="p-3 flex flex-col items-center justify-center text-center">
                    ...
                    <span className="text-xs" style={{ color: "#404823" }}>
                        {isMountainCity || cityName === "Кемер" ? (
                            [
                                stats.temperature900hPa !== undefined && `1000м ${stats.temperature900hPa}°`,
                                stats.temperature850hPa !== undefined && `, 1500м ${stats.temperature850hPa}°`
                            ].filter(Boolean).join(' ')
                        ) : (
...
const activeWeekend = activeTab === "w1" ? data.weekend1 : data.weekend2;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center sticky top-[68px] bg-[#edebe5] z-10 -mx-4 px-4 py-2 gap-4">
        <h2 className="text-sm font-bold text-slate-800 flex-1 min-w-0">
          {currentRouteData && routeStartCity !== routeEndCity
            ? `${routeStartCity} — ${routeEndCity}`
            : data.cityName}
        </h2>
        <button onClick={onClose} className="text-sm font-medium hover:text-[#3f5210] flex items-center gap-1 flex-shrink-0" style={{ color: "#4f6814" }}>
            ...
            {currentRouteData && (
              <div className="pt-3 flex flex-col sm:flex-row gap-2">
                <button
                  ...
                  className="flex items-center justify-center w-full p-4 bg-[rgb(223,219,207)] text-slate-800 rounded-lg font-bold hover:bg-[rgb(209,205,193)] transition-colors shadow-sm gap-2 text-center text-sm sm:text-base leading-tight"
                >
